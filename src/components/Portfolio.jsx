...
function ProjectTile({ project }) {
  return (
    <article className="group flex h-full flex-col rounded-[14px] border border-white/15 bg-black-900 p-6 transition-[transform,border-color] duration-200 ease-out hover:-translate-y-1 hover:border-white/30 md:p-8">
      <p className="font-display text-[11px] font-medium uppercase tracking-[0.22em] text-black-300">
        {project.category}
      </p>
      <h3 className="mt-3 font-display text-[22px] font-semibold leading-tight tracking-[-0.01em] text-cream">
        {project.name}
      </h3>
      <ul className="mt-4 flex flex-wrap gap-2">
        {project.tags.map((tag) => (
          <li
            key={tag}
            className="rounded-[4px] border border-white/15 px-3 py-1 text-[12px] text-black-300 transition-colors duration-500 ease-out group-hover:border-black-200 group-hover:bg-white group-hover:text-black-950"
          >
            {tag}
          </li>
        ))}
